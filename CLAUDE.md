...
- Per-target builders (`zed/`, `claude/`, `telegram/`, `terminal/`) are pure transformers: TOML in, target file out. No upstream JSON, no pipeline, no cross-builder imports.
...
```sh
make            # build all four targets
make zed        # one target at a time (also: claude, telegram, terminal)
make install    # build + copy/import into Zed, Claude, Terminal.app (Telegram is manual)
